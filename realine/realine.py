...
def _retrieve(i, j, s, S, T, str1, str2, out):
    ...
    if S[i, j] == 0:
        ...
        if j > 1 and S[i-1, j-2] + sigma_exp(str1[i-1], str2[j-2:j]) + s >= T:
            ...
            _retrieve(
                i-1, j-2, s+sigma_exp(str1[i-1], str2[j-2:j]), S, T, str1, str2, out)
        elif i > 1 and S[i-2, j-1] + sigma_exp(str2[j-1], str1[i-2:i]) + s >= T:
            out.insert(0, (str1[i-2:i], str2[j-1]))
            _retrieve(
                i-2, j-1, s+sigma_exp(str2[j-1], str1[i-2:i]), S, T, str1, str2, out)
            ...
            _retrieve(
                i-1, j-1, s+sigma_sub(str1[i-1], str2[j-1]), S, T, str1, str2, out)
    return out

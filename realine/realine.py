...
class ReAline(object):
    ...
    def sigma_exp(self, p, q):
        ...
        q1 = q[0]
        q2 = q[1]
        return self.C_exp - self.delta(p, q1) - self.delta(p, q2) - self.V(p) - max(self.V(q1), self.V(q2))

    def _retrieve(self,i, j, s, S, T, seq1, seq2, out):
        """
        ...
        if S[i, j] == 0:
            ...
            if j > 1 and S[i-1, j-2] + self.sigma_exp(seq1[i-1], seq2[j-2:j]) + s >= T:
                out.insert(0, (seq1[i-1], seq2[j-2:j]))
                self._retrieve(
                    i-1, j-2, s+self.sigma_exp(seq1[i-1], seq2[j-2:j]), S, T, seq1, seq2, out)
            elif i > 1 and S[i-2, j-1] + self.sigma_exp(seq2[j-1], seq1[i-2:i]) + s >= T:
                out.insert(0, (seq1[i-2:i], seq2[j-1]))
                self._retrieve(
                    i-2, j-1, s+self.sigma_exp(seq2[j-1], seq1[i-2:i]), S, T, seq1, seq2, out)
            elif S[i, j-1] + self.sigma_skip(seq2[j-1]) + s >= T:
                out.insert(0, ('-', seq2[j-1]))
                self._retrieve(i, j-1, s+self.sigma_skip(seq2[j-1]), S, T, seq1, seq2, out)
            elif S[i-1, j] + self.sigma_skip(seq1[i-1]) + s >= T:
                out.insert(0, (seq1[i-1], '-'))
                self._retrieve(i-1, j, s+self.sigma_skip(seq1[i-1]), S, T, seq1, seq2, out)
            elif S[i-1, j-1] + self.sigma_sub(seq1[i-1], seq2[j-1]) + s >= T:
                out.insert(0, (seq1[i-1], seq2[j-1]))
                self._retrieve(
                    i-1, j-1, s+self.sigma_sub(seq1[i-1], seq2[j-1]), S, T, seq1, seq2, out)
        return out

    def align(self, seq1: List[str], seq2: List[str], epsilon=0) -> List[List[Tuple[str, str]]]:
        """
        ...
        for i in range(1, m+1):
            for j in range(1, n+1):
                ...
                if i > 1:
                    ...
                    edit4 = -ReAline.inf
                if j > 1:
                    edit5 = S[i-1, j-2] + self.sigma_exp(seq1[i-1], seq2[j-2:j])
                else:
                    edit5 = -ReAline.inf

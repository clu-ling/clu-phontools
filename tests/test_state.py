...
class StateTests(unittest.TestCase):
    ...
    # for test purposes
    gold_a = Symbol(symbol="a", original_index=0, index=0, source=TranscriptTypes.GOLD,)
    gold_b = Symbol(symbol="b", original_index=1, index=1, source=TranscriptTypes.GOLD,)
    trans_a = Symbol(
        symbol="a", original_index=0, index=0, source=TranscriptTypes.TRANSCRIPT,
    )
    trans_b = Symbol(
        symbol="b", original_index=1, index=1, source=TranscriptTypes.TRANSCRIPT,
    )
    ...
    def test_ALIGN(self):
        ...
        state = State(
            stack=Stack(
                [
                    # top of stack
                    # NOTE: I fixed the top of the stack to be (gold)
                    StateTests.gold_a,
                    StateTests.trans_a,
                ]
            ),
    ...
    #     """`clu.phontools.alignment.parser.state.State` should support Actions.DISCARD."""

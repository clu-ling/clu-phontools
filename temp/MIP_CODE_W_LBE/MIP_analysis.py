...
# Calculate phoneme correctness
    ...
    return calcPhonemeErrorsFromAlignRes(pattern_phn_ipa, transcript_phn_ipa, align_res)

def calcPhonemeErrorsFromAlignRes(pattern_phn_ipa, transcript_phn_ipa, align_res):
...
def calcLBEfromAlignRes(pattern_stress,transcript_stress,align_res):
    # Calculate LBE
    #print pattern_stress
    #print transcript_stress
    # return LBE_IS,LBE_IW,LBE_DS,LBE_DW
                ...
                if _wrd_acc is None or '-' in transcript_phn:
                    ...
                    if _IS is not None:
                        ...
                        if pattern_stress[0][0]=='1':
                            SW_count += 1
                    _IS2,_IW2,_DS2,_DW2 = calcLBE(pattern_stress,transcript_stress)
                    IS2.append(_IS2)
                    IW2.append(_IW2)
                    DS2.append(_DS2)
                    DW2.append(_DW2)
                    if _IS2 is not None:
                        ...
                        if pattern_stress[0][0]=='1':
                            SW_count2 += 1
        spk.analysisResult = (wrd_acc,phn_acc,phn_ins,phn_del,phn_sub,total_phns,IS,IW,DS,DW,SW_count,WS_count, IS2,IW2,DS2,DW2,SW_count2,WS_count2)

        with open(spk.spkID + '.csv', 'w') as outF:
            outF.write(','.join(['target','transcript','word_acc','phn_correct','phn_ins','phn_del','phn_sub','total_phns','IS','IW','DS','DW','IS_kelvin','IW_kelvin','DS_kelvin','DW_kelvin']))
            outF.write('\n')
            for ind in range(len(wrd_acc)):
                outF.write(','.join(
                    [targets[ind], transcripts[ind],
                     ...
                     str(IS2[ind]), str(IW2[ind]), str(DS2[ind]), str(DW2[ind])]))
                outF.write('\n')
